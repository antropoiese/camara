...
var GexfJS = {
    ...
    i18n : {
        ...
        "pt" : {
            "search" : "Buscar nodo",
            "nodeAttr" : "Propriedades",
            "nodes" : "Nodos",
            "inLinks" : "Deputadas:",
            "outLinks" : "Temas:",
            "undirLinks" : "Arestas n&atilde;o direcionais com :",
            "lensOn" : "Ativar modo lupa",
            "lensOff" : "Desativar modo lupa",
            "edgeOn" : "Mostrar arestas",
            "edgeOff" : "Ocultar arestas",
            "zoomIn" : "Zoom In",
            "zoomOut" : "Zoom Out",
            "browserErr" : 'Seu navegador n&atilde;o &eacute; capaz de exibir esta p&aacute;gina corretamente.<br />Utilize a &uacute;ltima vers&atilde;o do <a href="http://www.mozilla.com/" target="_blank">Firefox</a> ou <a href="http://www.google.com/chrome/" target="_blank">Chrome</a>',
            "modularity_class" : "Classe de modularidade",
            "degree" : "Grau",
            "indegree" : "Grau de entrada",
            "outdegree" : "Grau de saída",
            "weighted degree" : "Grau ponderado",
            "weighted indegree" : "Grau de entrada ponderado",
            "weighted outdegree" : "Grau de saída ponderado",
            "closnesscentrality" : "Centralidade de proximidade",
            "betweenesscentrality" : "Centralidade de intermediação",
            "authority" : "Autoridade (HITS)",
            "hub" : "Hub (HITS)",
            "pageranks" : "Pontua&ccedil;o de PageRank",
            "edgeAttr" : "Propriedades",
        }
    },
    lang : "pt"
}
    ...
    if (_nodeIndex != -1) {
        var _d = GexfJS.graph.nodeList[_nodeIndex],
            ...
            _cG.animate({
                ...
                $("#aUnfold").attr("class","leftlinkArrayow");
                $("#zonecentre").css({
                    left: _cG.width() + "px"
                });
            });
        ...
        if (isNaN(_d.id))
          ...
          {
          _url = "http://facebook.com/";
          _rede = "Facebook";
          }
        _str += '<h3><div class="largepill" style="background: ' + _d.color.base +'"></div>' + _d.label + '</a></h3>';
        //_str += '<h3><div class="largepill" style="background: ' + _d.color.base +'"></div>' + _d.label + ' | <a href="' + _url+_d.id + '" target="_blank" title="Ver perfil no ' + _rede + '">ver perfil</a></h3>';

        // Propriedades largepill
        ...
        for (var i in GexfJS.graph.edgeList) {
            ...
            if ( _e.target == _nodeIndex ) {
		        ...
		        //str += '<ul><li><a href="' + _e.link + '" target="_blank">' + _e.link + '</a></li></ul>';
                linkArray.push(_e.link);
        ...
        for (var i = 0; i < linkArray.length; i++) {
            if (linkCount[linkArray[i]]) {
                linkCount[linkArray[i]] += 1;
            } else {
                linkCount[linkArray[i]] = 1;
            }
        }
        //for (i in linkCount) result += i + ' occurres ' + linkCount[i] + ' times\n';
        /*
        _str += '<div class="toggle"><h4 id="x">Mostrar Projetos de lei  [&plus;]</h4></div><div class="hide"><ul>'
        count = 1;
        for (i in linkCount) {
            //_str += '<li><a href="' + i + '" target="_blank">Projeto ' + count + '</a>: ' + linkCount[i] + ' compartilhamentos</li>';
            _str += '<li><a href="' + i + '" target="_blank">Projeto ' + count + '</a></li>';
            count += 1;
        }
        _str +='</div></ul>'
        //imprime script para expandir links
        _str += '<script type="text/javascript">$(document).ready(function () {\ntest = true;\n    $(".toggle").click(function () {\n        $(".hide").toggle();\n        if (test == true) {\n          x.innerHTML = \'Ocultar projetos de lei   [&minus;]\';\n          test = false;\n        }\n        else {\n          x.innerHTML = \'Mostrar projetos de lei  [&plus;]\';\n          test = true;\n        }\n    });\n});</script>'
        */
        // smallpill
        // Arestas de entrada
        ...
        for (var i in GexfJS.graph.edgeList) {
            ...
            if ( _e.target == _nodeIndex ) {
                ...
                _str += '<li><div class="smallpill" style="background: ' + _n.color.base +'"></div><a href="#" onmouseover="GexfJS.params.activeNode = ' + _e.source + '" onclick="displayNode(' + _e.source + ', true); return false;">' + _n.label + '</a>' + ( GexfJS.params.showEdgeWeight && _e.weight ? ' [' + _e.weight + ']' : '') + '</li>';
 		        //_str += '<ul><li><a href="' + _e.link + '" target="_blank">' + _e.link + '</a></li></ul>';
                //_str += '<li><a href="' + _e.link + '" target="_blank"><div class="smallpill" style="background: ' + _n.color.base +'"></div><a href="#" onmouseover="GexfJS.params.activeNode = ' + _e.source + '" onclick="displayNode(' + _e.source + ', true); return false;">' + _n.label + '</a>' + ( GexfJS.params.showEdgeWeight && _e.weight ? ' [' + _e.weight + ']' : '') + '</a></li>';
                //_str += '<li><div class="smallpill" style="background: ' + _n.color.base +'"></div><a href="#" onmouseover="GexfJS.params.activeNode = ' + _e.source + '" onclick="displayNode(' + _e.source + ', true); return false;">' + _n.label + '</a>' + ( GexfJS.params.showEdgeWeight && _e.weight ? ' [' + _e.weight + ']' : '') + ' | <a href="' + _e.link + '" target="_blank" title="Ver proposição">ver proposição</a></li>';
console.log(_e.link);
                            }
        ...
        for (var i in GexfJS.graph.edgeList) {
            ...
            if ( _e.source == _nodeIndex ) {
		        ...
		        //_str += '<ul><li><a href="' + _e.link + '" target="_blank">' + _e.link + '</a></li></ul>';
                //_str += '<li><a href="' + _e.link + '" target="_blank"><div class="smallpill" style="background: ' + _n.color.base +'"></div><a href="#" onmouseover="GexfJS.params.activeNode = ' + _e.target + '" onclick="displayNode(' + _e.target + ', true); return false;">' + _n.label + '</a>' + ( GexfJS.params.showEdgeWeight && _e.weight ? ' [' + _e.weight + ']' : '') + '</a></li>';
                //_str += '<li><div class="smallpill" style="background: ' + _n.color.base +'"></div><a href="#" onmouseover="GexfJS.params.activeNode = ' + _e.target + '" onclick="displayNode(' + _e.target + ', true); return false;">' + _n.label + '</a>' + ( GexfJS.params.showEdgeWeight && _e.weight ? ' [' + _e.weight + ']' : '') + ' | <a href="' + _e.link + '" target="_blank" title="Ver proposição">ver proposição</a></li>';
console.log(_e.link);
            }
        }
        _str += '</ul><p></p>';

        var _e = GexfJS.graph.edgeList[_nodeIndex];
        ...
        success: function(data) {
            ...
            $(_edges).each(function() {
                var _e = $(this),
                    ...
                    _col = _e.find("color");
                    _link = _e.find('attvalue[for="link"]').attr('value') || _e.attr('link');
                if (_col.length) {
                    var _r = _col.attr("r"),
                        _g = _col.attr("g"),
                        _b = _col.attr("b");
